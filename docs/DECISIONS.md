...
**Enforce:** `docs/scripts/dev/check-domain-neutral.sh` runs in CI.
...
**Enforce:** `docs/core/translator/pipeline.py` (jargon detector → simplifier → TL;DR).
...
**Enforce:** `docs/core/tools/base_tool.py` — `ToolResult.sources: list[str]` + `retrieved_at`.
...
> 3. `repo/docs/templates-us/<dept>/` (192 defaults from bb-plugin — lowest)
>
> Supports: `.md`, `.docx`, `.xlsx`. PDF optional.

**Enforce:** `docs/core/obsidian/template_resolver.py` — checks 3 paths in the correct order.
