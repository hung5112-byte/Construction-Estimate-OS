...
- bd_status reads Brain summary from fixture vault (no LLM)
- bd_run routes LLM calls via MCPSamplingProvider → ctx.session.create_message
"""
...
def test_mcp_server_imports():
    ...
    from core.mcp_server import (  # noqa: F401
        mcp,
        bd_run,
        bd_resume,
        bd_meeting,
...
def test_vn_status_returns_brain_summary(tmp_path):
    """bd_status reads Brain from fixture vault — pure, no LLM/session."""
    ...
    assert "retailers" in result["icp"]
...
def test_vn_run_routes_llm_via_mcp_sampling_provider(tmp_path):
    """bd_run must build FlowController with MCPSamplingProvider bound to ctx.session.
    ...
    fake_responses = [
        make_resp(json.dumps({
            "class": "SIMPLE",
            "departments": ["05-service-operations"],
            "reasoning": "SOP task",
        })),
    ...
    result = asyncio.run(bd_run(brief="Draft an RMA intake SOP", vault=str(vault), ctx=mock_ctx))
