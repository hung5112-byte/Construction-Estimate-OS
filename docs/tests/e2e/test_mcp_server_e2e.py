...
def test_mcp_vn_run_routes_via_sampling(tmp_path):
    ...
    ctx, mock_session = _make_ctx([
        '{"class": "SIMPLE", "departments": ["05-service-operations"], "reasoning": "SOP task"}',
...
def test_mcp_full_flow_brief_to_clarification_pause(tmp_path):
    ...
    ctx, mock_session = _make_ctx([
        ...
        json.dumps({
            "class": "COMPLEX",
            "departments": ["01-hardware-engineering", "02-npi-program-management"],
            "reasoning": "pilot launch",
        }),
        # Gap analysis: 1 gap
        json.dumps([{
            "field": "spares_plan", "severity": "CRITICAL",
            "current_value": "no spares planner", "brief_value": "pilot needs spares",
            "reason": "Brief diverges", "citation": "00-Brain/strategy.md",
        }]),
        # Question gen
        json.dumps([{
            "text": "Spares pool or advance replacement?",
            "citation": "00-Brain/strategy.md",
            "choices": ["Spares pool", "Advance replacement"],
...
def test_mcp_no_anthropic_api_key_needed(tmp_path, monkeypatch):
    ...
    ctx, mock_session = _make_ctx([
        '{"class": "SIMPLE", "departments": ["05-service-operations"], "reasoning": "x"}',
        '[]',
    ])
