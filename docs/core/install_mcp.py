...
def install(
    ...
    Args:
        ...
        vault_path: Optional vault to load <vault>/.env (TAVILY_API_KEY, ...)
                    and inject into the `env` of the mcpServers entry. Lets Claude
                    Desktop launch the MCP server with credentials ready — search tools
    ...
    # Inject env from vault/.env if vault_path is provided
