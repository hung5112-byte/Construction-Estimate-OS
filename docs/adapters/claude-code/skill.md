...
This skill uses **9 MCP tools** from the `vn-business-os` server. All LLM thinking runs through the **Claude Desktop / Code subscription** (no ANTHROPIC_API_KEY needed).
...
```bash
pip install vn-business-os         # or pipx install
vn-os install-mcp                  # auto-edits claude_desktop_config.json
# Restart Claude Desktop
```

After this, the `vn-business-os` MCP server is always available in Claude Desktop. This skill activates automatically when needed.

## 9 MCP Tools

| Tool | When to use |
|---|---|
| `vn_status(vault)` | Verify the vault exists + read the Brain summary |
| `vn_onboard(vault)` | Create a new vault scaffold (first time) |
| `vn_run(brief, vault)` | Stage 1: brief → router → gap → clarification |
| `vn_resume(task_folder)` | Stage 2: resume after the manager answers clarification |
| `vn_meeting(task_folder, departments)` | Stage 3: research + meeting → 07-decision-report.md |
| `vn_approve(task_folder)` | Stage 4: the manager approves → 08-execution-plan.md |
| `vn_execute(task_folder)` | Stage 5: render .docx/.xlsx into 03-Outputs/ |
| `vn_draft(brief, vault, doc_type)` | Fast path: one LLM call for boilerplate (agreement, JD, policy) — no meeting |
| `vn_upgrade(vault)` | Refresh an existing vault with updated prompts + aliases (does not touch Brain content) |

## Standard workflow

### Step 0: Detect vault + Brain
When the manager mentions a division task, read the CWD or ask for the vault path. Call `vn_status(vault)`:

```
✓ vault exists, ICP="mid-size US retailers", products=3, depts=["01-hardware-engineering","02-npi-program-management",...]
```

If the vault has no Brain → call `vn_onboard(vault)` to create the scaffold, and guide the manager to fill in 00-Brain/.

### Step 1: Run the task
```
vn_run(brief="Pilot 500 units of the new terminal with our top customer", vault="~/my-vault")
...
2. Call `vn_onboard(vault="~/vn-os/<slug>")`
...
4. After filling it in, call `vn_status(vault)` to verify.
...
- MCP tool error → tell the manager + suggest a fix:
  - Vault not found → run `vn_onboard()` first
...
- You can stop mid-session and resume later with `vn_resume(task_folder)` or `vn_meeting(task_folder)`
...
```
1. vn_status("~/division-vault") → ✓ Brain loaded
...
7. vn_meeting(task_folder) → all 5 departments debate, 07-decision-report.md ready
...
11. vn_execute(task_folder) → .docx/.xlsx into 03-Outputs/
