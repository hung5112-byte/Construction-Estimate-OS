...
departments/          # 12 core departments (YAML + agent .md)
packs/                # Industry packs (F&B, Retail, Tech-SaaS)
templates-us/         # 192 US/Texas-aligned templates
vault-template/       # Obsidian scaffold for onboarding
...
1. Create `core/tools/<name>.py` extending `BaseTool`
2. Implement `is_available()` (check key/dep)
3. Implement `run(query, **kwargs) -> ToolResult`
4. Register it in `core/orchestrator/research_phase.py:TOOL_REGISTRY`
